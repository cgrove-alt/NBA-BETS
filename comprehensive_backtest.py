...
# Import feature calculation functions from training script for consistency
...
class SeasonBacktester:
    ...
    def get_player_features_before_date(self, player_id: int, game_date: str,
        ...
        # ADD ADVANCED FEATURES USING SHARED FUNCTIONS FROM TRAINING SCRIPT
        # This ensures 100% consistency between training and backtest

        # 1. Pace-adjusted features (normalization for game tempo)
        team_pace = features.get('team_pace', 100.0)
        ...
        # 2. Vegas total features (game total projections)
        vegas_features = calculate_vegas_total_features(
            vegas_total=None,  # Historical backtest - no vegas data
            player_features=features,
            league_avg_total=225.0
        )
        features.update(vegas_features)

        # 3. Blowout risk features (predicts garbage time)
        # Note: Requires team stats not available in backtest context
        # Using default values for now (all zeros/neutrals)
        features.update({
        ...
        # 4. Regression-to-mean adjustment features (hot/cold streak correction)
    ...
    BIAS_CORRECTIONS = {
        'points': 2.280,  # Phase 2.5: Fix bias of -2.280
        'rebounds': 0.442,  # Phase 2.5: Fix bias of -0.442
        'assists': -1.543,  # Phase 2.5: Fix bias of 1.543
        'threes': 0.203,  # Phase 2.5: Fix bias of -0.203
        'pra': 0.676,  # Phase 2.5: Fix bias of -0.676
    }
