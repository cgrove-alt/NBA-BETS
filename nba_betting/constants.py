...
_FALLBACK_PROP_BIAS_CORRECTION: dict[str, float] = {
...
def _load_prop_bias_correction() -> dict[str, float]:
    """Load PROP_BIAS_CORRECTION from the calibration JSON, falling back to
    hardcoded values if the file is missing or malformed.

    This makes the constants data-driven: scripts/recalibrate_prop_bias.py
    regenerates the JSON after each retrain so the corrections track the
    current model's aggregate bias automatically.
    """
    json_path = Path(__file__).resolve().parent.parent / 'models' / 'calibration' / 'prop_bias_correction.json'
    try:
        with open(json_path, encoding='utf-8') as f:
            payload = json.load(f)
        corrections = payload.get('corrections')
        if not isinstance(corrections, dict):
            raise ValueError("JSON missing 'corrections' dict")
        # Type-coerce and require every fallback key to be present
        result: dict[str, float] = {}
        for prop, fallback in _FALLBACK_PROP_BIAS_CORRECTION.items():
            val = corrections.get(prop, fallback)
            result[prop] = float(val)
        return result
    except FileNotFoundError:
        _log.info("prop_bias_correction.json not found — using hardcoded fallback values")
        return dict(_FALLBACK_PROP_BIAS_CORRECTION)
    except Exception as exc:  # noqa: BLE001 — never crash module import on bad calibration data
        _log.warning("Failed to load prop_bias_correction.json (%s) — using fallback", exc)
        return dict(_FALLBACK_PROP_BIAS_CORRECTION)


PROP_BIAS_CORRECTION: dict[str, float] = _load_prop_bias_correction()

# ---------------------------------------------------------------------------
# Disabled prop types (no demonstrated model edge)
# ---------------------------------------------------------------------------
# Loaded from models/calibration/prop_enable_gates.json, which is regenerated
# after each backtest by scripts/recalibrate_prop_gates.py based on objective
# criteria (bootstrap significance, hit rate, RMSE vs naive baseline). Hardcoded
# fallback below preserves the 2026-03-22 statistical-analysis configuration:
#   - Bootstrap significance: rebounds p=0.027 (significant), PRA p=0.068.
#   - Points p=0.463 → no edge.
#   - Assists: 45.3% hit rate, no fixable directional correction.
#   - Threes too stochastic; spread RMSE 14.2 > market ~12-13.
#
...
_FALLBACK_DISABLED_PROPS: list[str] = ['points', 'assists', 'threes', 'spread']


def _load_disabled_props() -> list[str]:
    """Load DISABLED_PROPS from the calibration JSON.

    The JSON is regenerated by scripts/recalibrate_prop_gates.py after each
    OOS walk-forward run and applies these objective gates to each prop:
      - Bootstrap significance vs naive baseline must reach p < 0.05.
      - Trade win rate (when sampled) must clear the -110 break-even (52.4%).
      - RMSE must not be worse than the naive (season-average) baseline.
    A prop that fails any gate is added to disabled. Conversely, a prop in the
    fallback list that now passes all gates is *automatically re-enabled* —
    no code change required. This is the path forward after retraining with
    the under-fix and survivorship-fix from this audit.

    Missing/malformed JSON falls back to the hardcoded conservative list.
    """
    json_path = Path(__file__).resolve().parent.parent / 'models' / 'calibration' / 'prop_enable_gates.json'
    try:
        with open(json_path, encoding='utf-8') as f:
            payload = json.load(f)
        disabled = payload.get('disabled_props')
        if not isinstance(disabled, list):
            raise ValueError("JSON missing 'disabled_props' list")
        # Type-coerce and dedupe
        return sorted({str(p).strip().lower() for p in disabled if p})
    except FileNotFoundError:
        _log.info("prop_enable_gates.json not found — using hardcoded fallback list")
        return list(_FALLBACK_DISABLED_PROPS)
    except Exception as exc:  # noqa: BLE001
        _log.warning("Failed to load prop_enable_gates.json (%s) — using fallback", exc)
        return list(_FALLBACK_DISABLED_PROPS)


DISABLED_PROPS: list[str] = _load_disabled_props()
